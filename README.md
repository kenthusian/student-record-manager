...
A simple Python application to manage student records for a school. Records are stored in a single CSV file (students.csv). The code supports adding, viewing, updating, and deleting student records, plus a data analytics report using NumPy and Pandas.

This project is split into two parts as per the BACSE101 course requirements:

Part 1 (Fundamentals): A menu-driven system for record management (main.py).

Part 2 (Analytics): A data analytics module using NumPy and Pandas (analytics.py).
...
Save the three files (main.py, analytics.py, students.csv) in a single project folder.
...
In your terminal, run the main.py script:

python main.py


The application menu will appear, and you can now use the program.
...
Example Row: 1001,Raj Sharma,10-A,85,90,78

Project Structure

main.py

Contains the main CLI application, menu, and all record management functions (add_student, view_student, update_grades, delete_student).
